...
const AddPositionToStoragePopup = () => {
    ...
    const [dataSection, setDataMenu] = useState({name: "", description: "", price: 0, weigth: ""})

    function addPosition(evt: any) {
        evt.preventDefault();
        dispatch(addPositionToStorage({...dataSection, id: uuid4()}))
    }

    return (
        <div className="popupContainer">
            <input type="text" className="input" required minLength={4} placeholder="название позиции" value={dataSection.name} onChange={(e) => {setDataMenu({...dataSection, name: e.target.value})}}/>
